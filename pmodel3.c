...
int main(int argc, char ** argv){
	...
	mb_A = 1;
	nb_A = 1;
    mb_b = mb_A;    //POSSIBLE WEAKNESS: We want b to have the same number of rows as A_distr (Is this the way to do it?)
    nb_b = 1;
    ...
    nq_b = 1; //numroc_(&i_one, &nb_b, &pcol, &i_zero, &npcol);
	...
	lld_A = MAX( numroc_( &M, &M, &prow, &i_zero, &nprow ), 1 );
    lld_A_distr = MAX(mp_A, 1);
    lld_b = MAX(numroc_(&M, &M, &prow, &i_zero, &nprow), 1); 
    ...
    */
    ...
    double temp_work[1];
    pdgels_("N", &mp_A, &nq_A, &i_one, A_distr, &ia, &ja, descA_distr, b_distr, &ib, &jb, descb_distr, temp_work, &i_neg_one, &info);
    if(info!=0){
    ...
    pdgels_("N", &mp_A, &nq_A, &i_one, A_distr, &ia, &ja, descA_distr, b_distr, &ib, &jb, descb_distr, work, &i_neg_one, &info);
    if(info!=0){

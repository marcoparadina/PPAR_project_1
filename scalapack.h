...
extern void Cpdgemr2d(long m, long n, double *A, int IA, int JA, int *descA, double *B, int IB, int JB, int *descB, int gcontext);


int numroc_(long* N, long* NB, int* IPROC, int *ISRCPROC, int* NPROCS );
...
void descinit_ (int *desc, const long *m, const long *n, const long *mb, const long *nb, const int *irsrc, const int *icsrc, const int *ictxt, const long *lld, int *info);
void pdgels_(char* TRANS, long* M, long* N, int* NRHS, double *A, int * IA, int * JA, int * DESCA, double * B, int * IB, int * JB, int * DESCB, double * WORK, long * LWORK, int * INFO);
